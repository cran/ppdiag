test_that("test pp_simulate", {
  ...
  expect_message(
    pp_simulate(pp_hpp(lambda = 1), start = 2, end = 3, verbose = TRUE),
    "Simulating up to endtime. To simulate n events specify n."
  )
  expect_message(
    pp_simulate(pp_hpp(lambda = 1, events = c(1)),
      start = 2,
      end = 3, verbose = TRUE
    ),
  ...
  expect_error(
    pp_simulate(pp_hpp(lambda = 1), n = 0),
    "n must be positive for simulation"
  )
  expect_error(
    pp_simulate(pp_hpp(lambda = 1), n = -2),
    "n must be positive for simulation"
  )
  expect_error(
    pp_simulate(pp_hpp(lambda = 1), start = 0, end = NULL),
  ...
  expect_message(
    pp_simulate(hp_obj, start = 2, end = 3, n = 10, verbose = TRUE),
    "10 events simulated. To simulate up to endtime set n=NULL."
  )
  expect_message(
    pp_simulate(hp_obj, start = 2, end = 3, verbose = TRUE),
    "Simulating up to endtime. To simulate n events specify n."
  )
  expect_null(pp_simulate(hp_obj, start = 2, end = 2))
  expect_null(pp_simulate(hp_obj, start = 2, end = 1))
  expect_error(
    pp_simulate(hp_obj, start = 2, end = 10, n = 0),
    "n must be positive for simulation."
  )
  expect_error(
    pp_simulate(hp_obj, start = 2, end = 10, n = -2),
    "n must be positive for simulation."
  )
